...
def main():
    ...
    occuring_seats = [get_seat_id(board_pass) for board_pass in board_passes]
    min_seat = min(occuring_seats)
    max_seat = max(occuring_seats)
    possible_solutions = []
    for i in range(min_seat, max_seat):
        if i not in occuring_seats and i-1 in occuring_seats and i + 1 in occuring_seats:
